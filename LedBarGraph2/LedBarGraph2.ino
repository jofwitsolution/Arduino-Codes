...
const int decay = 1000; // increasing this reduces decay rate of storedValue
...
storedValue = max(sensorValue, storedValue); // only use sensor value if higher
ledLevel = map(storedValue, 0, 1023, 0, NbrLEDs); // map to number of LEDs
Serial.println(NbrLEDs);
